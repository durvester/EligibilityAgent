...
3. **Pattern matching**: Backend selects OAuth credentials based on issuer URL (practicefusion.com → PF, allscripts.com → VERADIGM)
4. If PKCE enabled for this EHR, generates `code_verifier`/`code_challenge` (S256) and appends challenge to authorize URL
5. User authorizes, callback exchanges code for token using EHR-specific credentials (+ `code_verifier` if PKCE)
6. Backend creates tenant (issuer = tenant identifier) and session with `ehrIdentifier`
7. Internal JWT stored in HttpOnly cookie, OAuth tokens encrypted in database
8. Token refresh happens on-demand via retry-on-401 pattern using `ehrIdentifier` to select credentials (PKCE not involved in refresh)
...
### Multi-EHR OAuth Support

**Pattern Matching**:
- `practicefusion.com` → Uses `PF_CLIENT_ID`, `PF_CLIENT_SECRET`, `PF_SCOPES`
- `allscripts.com` → Uses `VERADIGM_CLIENT_ID`, `VERADIGM_CLIENT_SECRET`, `VERADIGM_SCOPES`
- Unknown issuer → Returns 400 error (NO FALLBACKS)

**Session Management**:
- `ehrIdentifier` stored in session during creation ('PF', 'VERADIGM')
- Token refresh uses `ehrIdentifier` to build env var names dynamically: `${ehrIdentifier}_CLIENT_ID`
- If `ehrIdentifier` is NULL, token refresh fails explicitly (forces re-login)

**PKCE (Proof Key for Code Exchange)**:
- Opt-in per EHR via `{PREFIX}_USE_PKCE=true` env var (default: off)
- Uses S256 challenge method per RFC 7636
- `code_verifier` stored in in-memory `stateStore` alongside launch state (cleaned up by existing 10-min TTL)
- Both GET and POST `/callback` handlers conditionally include `code_verifier` in token exchange
- Only affects authorization code flow; token refresh is unaffected
- Implementation: `apps/api/src/lib/pkce.ts` (two pure functions using Node.js `crypto`)

**Adding New EHR Systems**:
1. Add pattern to `EHR_CONFIGS` in `apps/api/src/services/ehr-credentials.ts`
2. Set environment variables: `{PREFIX}_CLIENT_ID`, `{PREFIX}_CLIENT_SECRET`, `{PREFIX}_SCOPES`
3. Optionally set `{PREFIX}_USE_PKCE=true` if the EHR supports PKCE
...
- **OAuth Tokens**: Encrypted (AES-256-GCM) in PostgreSQL, ONLY for FHIR calls
- **Token Retry**: FHIR proxy uses retry-on-401 - refresh token via stored `tokenEndpoint` and `ehrIdentifier`, retry once
...
| File | Purpose |
|------|---------|
| `apps/api/src/services/ehr-credentials.ts` | Multi-EHR OAuth credential selection (pattern matching) |
| `apps/api/src/routes/auth.ts` | SMART OAuth with dynamic discovery and multi-EHR support |
...
| `apps/api/src/lib/pkce.ts` | PKCE code verifier/challenge generation (RFC 7636) |
...
### New Agent Tool
1. Add tool definition to `TOOLS` array in `apps/api/src/services/agent/loop.ts`
...
Current tools: `lookup_npi`, `search_npi`, `search_payers`, `check_eligibility`, `discover_insurance`
